...
class API_Creation {
		...
		if ( isset( $_GET['_wpnonce'] ) && check_admin_referer( 'MO_CAW_API_Creation_' . ucfirst( $action ) . '_Nonce', '_wpnonce' ) ) {
			...
			if ( ( Constants::EDIT === $action || Constants::VIEW === $action || Constants::TEST === $action || Constants::DELETE === $action ) && Constants::DISABLED !== $this->license_status ) {
				...
				if ( empty( $this->invalid_request_message ) ) {
					...
					$this->gui_endpoint_config = empty( $this->gui_endpoint_config ) ? DB_Utils::get_single_configuration( $row_filter ) : $this->gui_endpoint_config;

					if ( empty( $this->gui_endpoint_config ) ) {
	...
	private function display_invalid_request( $message ) {
		...
		<a class="btn btn-primary mo-caw-rounded-16 mo-caw-bg-blue-dark px-4" href="<?php echo esc_url( 'admin.php?page=custom_api_wp_settings&tab=custom-api' ); ?>">Back to Custom APIs</a>
		<?php
	}
